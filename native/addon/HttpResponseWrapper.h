...
/* PROTOCOL is 0 = TCP, 1 = TLS */

struct HttpResponseWrapper {

    static bool hasHeaderControlCharacters(std::string_view value, bool allowTab = false) {
    ...
    static void throwHeaderControlCharacters(const FunctionCallbackInfo<Value> &args) {
        Isolate *isolate = args.GetIsolate();
        args.GetReturnValue().Set(isolate->ThrowException(v8::Exception::Error(String::NewFromUtf8(isolate, "uWS: header contains control characters", NewStringType::kNormal).ToLocalChecked())));
    }

    static void assumeCorked() {
        static bool warned = false;
        if (!insideCorkCallback && !warned) {
    ...
    static void res_collectBody(const FunctionCallbackInfo<Value> &args) {
        ...
        if (res) {
            ...
            double maxSizeNumber = args[0]->NumberValue(isolate->GetCurrentContext()).FromMaybe(0);
            if (!std::isfinite(maxSizeNumber) || maxSizeNumber < 0) {
                args.GetReturnValue().Set(isolate->ThrowException(v8::Exception::Error(String::NewFromUtf8(isolate, "Expected a number.", NewStringType::kNormal).ToLocalChecked())));
                return;
            }
            if (!requireFunction(args, 1)) return;
            size_t maxSize = (size_t) maxSizeNumber;
    ...
    static void res_writeStatus(const FunctionCallbackInfo<Value> &args) {
        auto *res = getHttpResponse<SSL>(args);
            ...
            if (hasHeaderControlCharacters(data.getString())) {
                throwHeaderControlCharacters(args);
                return;
            }
    ...
    static void res_endWithoutBody(const FunctionCallbackInfo<Value> &args) {
        ...
        if (res) {
            ...
            if (args.Length() >= 1 && !args[0]->IsUndefined()) {
                ...
                reportedContentLength = (size_t) args[0]->NumberValue(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
            }
    ...
    static void res_tryEnd(const FunctionCallbackInfo<Value> &args) {
        ...
        if (res) {
            ...
            if (args.Length() > 1 && !args[1]->IsUndefined()) {
                ...
                totalSize = (size_t) args[1]->NumberValue(isolate->GetCurrentContext()).FromMaybe(0);
            }
            ...
            /* Packed flags: bit 0 = ok, bit 1 = hasResponded */
            args.GetReturnValue().Set(Integer::New(isolate, (ok ? 1 : 0) | (hasResponded ? 2 : 0)));
        }
    }
    ...
    static void res_writeHeader(const FunctionCallbackInfo<Value> &args) {
        ...
        if (res) {
            ...
            if (header.getString().find(':') != std::string_view::npos || hasHeaderControlCharacters(header.getString()) || hasHeaderControlCharacters(value.getString(), true)) {
                throwHeaderControlCharacters(args);
                return;
            }
            assumeCorked();
    ...
    static void res_upgrade(const FunctionCallbackInfo<Value> &args) {
        ...
        if (res) {
            ...
            if (args.Length() != 5) {
                args.GetReturnValue().Set(isolate->ThrowException(v8::Exception::Error(String::NewFromUtf8(isolate, "Function requires exactly 5 arguments.", NewStringType::kNormal).ToLocalChecked())));
                return;
            }
            ...
            if (secWebSocketKey.getString().length() != 24) {
                args.GetReturnValue().Set(isolate->ThrowException(v8::Exception::Error(String::NewFromUtf8(isolate, "Sec-WebSocket-Key must be exactly 24 bytes.", NewStringType::kNormal).ToLocalChecked())));
                return;
            }
            ...
            if (!args[4]->IsExternal()) {
                args.GetReturnValue().Set(isolate->ThrowException(v8::Exception::Error(String::NewFromUtf8(isolate, "uWS: res.upgrade() requires the WebSocketContext as its 5th argument", NewStringType::kNormal).ToLocalChecked())));
                return;
            }
