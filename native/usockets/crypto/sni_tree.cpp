...
extern "C" {
    ...
    /* Returns 1 if this name already exists or exceeds MAX_LABELS, otherwise 0 */
    int sni_add(void *sni, const char *hostname, void *user) {
        struct sni_node *root = (struct sni_node *) sni;

        std::string_view labels[MAX_LABELS];
        ...
        /* Insert only after all labels have been validated */
