...
/* Not used for polls, since polls have their own allocation layout */
static void close_cb_free(uv_handle_t *h) {
    free(h->data);
}

/*
 * Keep the allocation size before the poll. This lets resize copy the old
 * extension while retaining the old, registered uv_poll_t until uv_close has
 * completed. The aligned member makes the following us_poll_t retain its
 * required extension alignment.
 */
struct us_poll_allocation_t {
    alignas(LIBUS_EXT_ALIGNMENT) unsigned int ext_size;
};

static struct us_poll_allocation_t *us_poll_allocation(struct us_poll_t *p) {
    return ((struct us_poll_allocation_t *) p) - 1;
}

/* This is different for polls: the uv handle and poll share one allocation. */
static void close_cb_free_poll(uv_handle_t *h) {
    ...
    if (h->data) {
        free(us_poll_allocation((struct us_poll_t *) h->data));
    }
}
...
void us_poll_free(struct us_poll_t *p, struct us_loop_t *loop) {
    /* us_poll_stop clears data so its close callback keeps the allocation alive. If this is the
     * deferred free while that close is pending, restore data for close_cb_free_poll to free it. */
    if (uv_is_closing((uv_handle_t *) p->uv_p)) {
        p->uv_p->data = p;
    } else {
        free(us_poll_allocation(p));
    }
}
...
struct us_loop_t *us_create_loop(void *hint, void (*wakeup_cb)(struct us_loop_t *loop), void (*pre_cb)(struct us_loop_t *loop), void (*post_cb)(struct us_loop_t *loop), unsigned int ext_size) {
    ...
    // Node always hands us its loop, so we always integrate and set up the timer
...
struct us_poll_t *us_create_poll(struct us_loop_t *loop, int fallthrough, unsigned int ext_size) {
    struct us_poll_allocation_t *allocation = malloc(sizeof(struct us_poll_allocation_t) + sizeof(struct us_poll_t) + ext_size + sizeof(uv_poll_t));
    if (!allocation) {
        return 0;
    }
    struct us_poll_t *p = (struct us_poll_t *) (allocation + 1);

    allocation->ext_size = ext_size;
    p->uv_p = (uv_poll_t *) ((char *) p + sizeof(struct us_poll_t) + ext_size);
    p->uv_p->data = p;
    return p;
}

/*
 * A registered libuv handle is an intrusive node in the loop's handle queue,
 * so it must never be relocated by realloc. Close the old handle at its old
 * address, retain that allocation for its asynchronous close callback, and
 * start a fresh handle in the replacement allocation instead.
 */
struct us_poll_t *us_poll_resize(struct us_poll_t *p, struct us_loop_t *loop, unsigned int ext_size) {
    struct us_poll_allocation_t *old_allocation = us_poll_allocation(p);
    unsigned int old_ext_size = old_allocation->ext_size;
    unsigned int events = us_poll_events(p);
    uv_poll_t *old_uv_p = p->uv_p;
    struct us_poll_allocation_t *new_allocation = malloc(sizeof(struct us_poll_allocation_t) + sizeof(struct us_poll_t) + ext_size + sizeof(uv_poll_t));
    if (!new_allocation) {
        return p;
    }
    struct us_poll_t *new_p = (struct us_poll_t *) (new_allocation + 1);

    memcpy(new_p, p, sizeof(struct us_poll_t) + (old_ext_size < ext_size ? old_ext_size : ext_size));
    new_allocation->ext_size = ext_size;
    new_p->uv_p = (uv_poll_t *) ((char *) new_p + sizeof(struct us_poll_t) + ext_size);

    uv_poll_stop(old_uv_p);
    old_uv_p->data = p;
    uv_close((uv_handle_t *) old_uv_p, close_cb_free_poll);

    uv_poll_init_socket(loop->uv_loop, new_p->uv_p, new_p->fd);
    new_p->uv_p->data = new_p;
    uv_poll_start(new_p->uv_p, events, poll_cb);
