...
void us_socket_context_close(int ssl, struct us_socket_context_t *context) {
    ...
    while (context->head_sockets) {
        struct us_socket_t *s = context->head_sockets;
        /* A connecting (not-yet-established) socket never got on_open, so it must not emit
         * on_close; route it through us_socket_close_connecting instead. */
...
/* Low-priority sockets are temporarily unlinked from context->head_sockets. Close matching ones
 * separately before freeing a context, since they still retain its pointer through their poll. */
void us_internal_socket_context_close_low_prio(int ssl, struct us_socket_context_t *context) {
    struct us_socket_t *s;

    while (1) {
        for (s = context->loop->data.low_prio_head; s && s->context != context; s = s->next) {
        }
        if (!s) {
            break;
        }
...
/* Clear userdata attached to an SNI context without removing the context itself. */
void us_socket_context_clear_server_name_userdata(int ssl, struct us_socket_context_t *context, const char *hostname_pattern) {
#ifndef LIBUS_NO_SSL
    if (ssl) {
        extern void us_internal_ssl_socket_context_clear_server_name_userdata(struct us_internal_ssl_socket_context_t *context, const char *hostname_pattern);
        us_internal_ssl_socket_context_clear_server_name_userdata((struct us_internal_ssl_socket_context_t *) context, hostname_pattern);
    }
#endif
}
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        extern int us_internal_ssl_socket_context_add_server_name_with_status(struct us_internal_ssl_socket_context_t *context, const char *hostname_pattern, struct us_socket_context_options_t options, void *user);
        return us_internal_ssl_socket_context_add_server_name_with_status((struct us_internal_ssl_socket_context_t *) context, hostname_pattern, options, user);
    }
#endif
    return -1;
}
...
struct us_socket_context_t *us_create_socket_context(int ssl, struct us_loop_t *loop, int context_ext_size, struct us_socket_context_options_t options) {
    ...
    /* No in-flight connect requests yet */
    context->head_connect_requests = 0;
...
void us_socket_context_free(int ssl, struct us_socket_context_t *context) {
    ...
    /* Tear down any connect requests still bound to this context. A request whose uv_getaddrinfo is
     * still in flight is cancelled and orphaned (context set to NULL); its callback fires on a later
     * loop turn, sees context == NULL and frees the request without touching this freed context. This
     * is the ASAN-critical orphaning invariant - see connect.c. */
    struct us_connect_request_t *r = context->head_connect_requests;
    while (r) {
        struct us_connect_request_t *next_r = us_internal_connect_request_next(r);
        us_internal_connect_request_orphan(r);
        r = next_r;
    }
    context->head_connect_requests = 0;
