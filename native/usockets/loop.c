...
void us_internal_loop_data_init(struct us_loop_t *loop, void (*wakeup_cb)(struct us_loop_t *loop),
    ...
    loop->data.accept_timer = us_create_timer(loop, 1, 0);
    loop->data.paused_listen_sockets = 0;
...
void us_internal_loop_data_free(struct us_loop_t *loop) {
    ...
    us_timer_close(loop->data.accept_timer);
    while (loop->data.paused_listen_sockets) {
        struct us_paused_listen_socket_t *next = loop->data.paused_listen_sockets->next;
        free(loop->data.paused_listen_sockets);
        loop->data.paused_listen_sockets = next;
    }
...
int us_internal_loop_pending_resolves(struct us_loop_t *loop) {
    return loop->data.pending_resolves;
}
...
static int is_live_listen_socket(struct us_loop_t *loop, struct us_listen_socket_t *listen_socket) {
    struct us_socket_context_t *context;

    for (context = loop->data.head; context; context = context->next) {
        struct us_listen_socket_t *candidate;

        for (candidate = context->head_listen_sockets; candidate; candidate = (struct us_listen_socket_t *) candidate->s.next) {
            if (candidate == listen_socket) {
                return 1;
            }
        }
    }
...
static void accept_timer_cb(struct us_timer_t *timer) {
    struct us_loop_t *loop = us_timer_loop(timer);
    struct us_paused_listen_socket_t *paused_listen_sockets = loop->data.paused_listen_sockets;

    loop->data.paused_listen_sockets = 0;
    while (paused_listen_sockets) {
        struct us_paused_listen_socket_t *next = paused_listen_sockets->next;

        /* The listen socket may have been closed while the timer was pending. */
        if (is_live_listen_socket(loop, paused_listen_sockets->listen_socket)) {
            us_poll_change(&paused_listen_sockets->listen_socket->s.p, loop, LIBUS_SOCKET_READABLE);
        }

        free(paused_listen_sockets);
        paused_listen_sockets = next;
    }
}

static int accept_error_is_fd_exhaustion() {
#ifdef _WIN32
    return bsd_socket_last_error() == WSAEMFILE;
#else
    int error = bsd_socket_last_error();
    return error == EMFILE || error == ENFILE;
#endif
}

static void pause_listen_socket(struct us_loop_t *loop, struct us_listen_socket_t *listen_socket) {
    struct us_paused_listen_socket_t *paused_listen_socket = malloc(sizeof(struct us_paused_listen_socket_t));

    if (paused_listen_socket) {
        int was_empty = loop->data.paused_listen_sockets == 0;

        paused_listen_socket->listen_socket = listen_socket;
        paused_listen_socket->next = loop->data.paused_listen_sockets;
        loop->data.paused_listen_sockets = paused_listen_socket;
        us_poll_change(&listen_socket->s.p, loop, 0);

        if (was_empty) {
            us_timer_set(loop->data.accept_timer, accept_timer_cb, 1000, 0);
        }
    }
}
...
void us_internal_dispatch_ready_poll(struct us_poll_t *p, int error, int events) {
    ...
    case POLL_TYPE_SEMI_SOCKET: {
            ...
            if (us_poll_events(p) == LIBUS_SOCKET_WRITABLE) {
                ...
                /* It is perfectly possible to come here with an error. libuv delivers a writable
                 * event for a failed non-blocking connect without a poll error, so read SO_ERROR. */
                int err = 0;
                socklen_t err_len = sizeof(err);
                getsockopt(us_poll_fd(p), SOL_SOCKET, SO_ERROR, (void *) &err, &err_len);
                /* SO_ERROR yields WSAE* codes on Windows; normalize to POSIX errno so on_connect_error
                 * reports ECONNREFUSED/ETIMEDOUT/... rather than a fallback ECONNRESET. No-op elsewhere. */
                err = bsd_normalize_socket_error(err);

                /* A poll error without an SO_ERROR still means the connect failed. */
                if (error && !err) {
                    err = ECONNREFUSED;
                }

                /* connect.c owns the request state machine: on failure it retries the next address or
                 * emits on_connect_error, on success it promotes the socket and fires on_open. */
                us_internal_socket_connect_ready(s, err);
            } else {
                ...
                if (client_fd == LIBUS_SOCKET_ERROR) {
                    if (accept_error_is_fd_exhaustion()) {
                        pause_listen_socket(us_socket_context(0, &listen_socket->s)->loop, listen_socket);
                    }
                    ...
                    if (client_fd == LIBUS_SOCKET_ERROR && accept_error_is_fd_exhaustion()) {
                        pause_listen_socket(us_socket_context(0, &listen_socket->s)->loop, listen_socket);
                    }
                }
            }
