...
private:
    ...
    /* Write an unsigned 64-bit integer in hex */
    void writeUnsignedHex(uint64_t value) {
        char buf[16];
        char *end = buf + sizeof(buf);
        char *start = end;

        do {
            *--start = "0123456789abcdef"[value & 15];
            value >>= 4;
        } while (value);

        /* For now we do this copy */
        Super::write(start, (int) (end - start));
    }
    ...
    bool internalEnd(std::string_view data, uintmax_t totalSize, bool optional, bool allowContentLength = true, bool closeConnection = false) {
        ...
        /* HEAD responses retain their would-be Content-Length but never send a response body. */
        if (httpResponseData->state & HttpResponseData<SSL>::HTTP_METHOD_HEAD) {
            data = {};
        }
        ...
        if (httpResponseData->state & HttpResponseData<SSL>::HTTP_WRITE_CALLED) {
            ...
            if (httpResponseData->state & HttpResponseData<SSL>::HTTP_METHOD_HEAD) {
                /* Terminate the header block without emitting chunked body framing. */
                Super::write("\r\n", 2);
            } else {
...
public:
    ...
    void upgrade(UserData &&userData, std::string_view secWebSocketKey, std::string_view secWebSocketProtocol,
        ...
        /* An upgraded socket leaves the HTTP context before it is adopted by the WebSocket context */
        for (auto &f : httpContextData->filterHandlers) {
            f(this, -1);
        }

        /* Move socket data out of HttpResponse before adopting the socket */
        AsyncSocketData<SSL> asyncSocketData(std::move(*(AsyncSocketData<SSL> *) getHttpResponseData()));
        ...
        WebSocket<SSL, true, UserData> *webSocket = (WebSocket<SSL, true, UserData> *) us_socket_context_adopt_socket(SSL,
                    (us_socket_context_t *) webSocketContext, (us_socket_t *) this, sizeof(WebSocketData<true>) + sizeof(UserData));
        ...
        /* Initialize websocket with any moved socket data intact */
        webSocket->init(perMessageDeflate, compressOptions, std::move(asyncSocketData));
    ...
    bool write(std::string_view data) {
        ...
        /* HEAD responses must not emit chunked body data. */
        if (httpResponseData->state & HttpResponseData<SSL>::HTTP_METHOD_HEAD) {
            return true;
        }
    ...
    bool hasOnAborted() {
        return (bool) getHttpResponseData()->onAborted;
    }

    /* Arm the idle timeout for a response whose handler returned without responding
     * (used with the default abort handler so a never-responding connection still closes). */
    void armIdleTimeout() {
        Super::timeout(HTTP_TIMEOUT_S);
    }
