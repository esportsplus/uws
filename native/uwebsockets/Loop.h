...
private:
    ...
    static void preCb(us_loop_t *loop) {
        ...
        for (void *key : keys) {
            auto handler = loopData->preHandlers.find(key);
            if (handler != loopData->preHandlers.end()) {
                handler->second((Loop *) loop);
            }
        }
    }
    ...
    static void postCb(us_loop_t *loop) {
        ...
        for (void *key : keys) {
            auto handler = loopData->postHandlers.find(key);
            if (handler != loopData->postHandlers.end()) {
                handler->second((Loop *) loop);
            }
        }
