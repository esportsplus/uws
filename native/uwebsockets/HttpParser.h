...
public:
    ...
    std::string_view getMethod() {
        ...
        methodValue.assign(headers->key.data(), headers->key.length());
        for (unsigned int i = 0; i < methodValue.length(); i++) {
            methodValue[i] |= 32;
        }
    ...
    std::string_view getQuery(std::string_view key) {
        ...
        return getDecodedQueryValue(key, queryString, queryValue);
    }
...
private:
    std::string fallback;
    /* The lower 60 bits hold the size; the four highest bits are reserved for chunked encoding parsing state. */
    uint64_t remainingStreamingBytes = 0;
    unsigned char trailerTail = 0;

    void consumeChunkedTrailer(std::string_view &data) {
        while (data.length() && remainingStreamingBytes == STATE_IS_TRAILER) {
            unsigned char c = (unsigned char) data[0];
            data.remove_prefix(1);

            /* The first CRLF after the zero chunk ends a body without trailers. */
            if (trailerTail == 0) {
                if (c == '\r') {
                    trailerTail = 4;
                } else {
    ...
    /* Returns UINT64_MAX on error. Maximum 999999999999999999 (18 digits) is allowed. */
    ...
    static unsigned int getHeaders(char *postPaddedBuffer, char *end, struct HttpRequest::Header *headers, void *reserved, unsigned int &err) {
        ...
        for (unsigned int i = 1; i < UWS_HTTP_MAX_HEADERS_COUNT - 1; i++) {
            ...
            if (postPaddedBuffer[1] == '\n') {
                ...
                while (headers->value.length() && (unsigned char) headers->value.back() < 33) {
                    headers->value.remove_suffix(1);
                }

                /* Trim initial whitespace (SP, HTAB) */
                while (headers->value.length() && (unsigned char) headers->value.front() < 33) {
        ...
        for (unsigned int consumed; length && (consumed = getHeaders(data, data + length, req->headers, reserved, err)); ) {
            ...
            for (HttpRequest::Header *h = req->headers; (++h)->key.length(); ) {
                if (req->bf.mightHave(h->key)) [[unlikely]] {
                    /* Host, Content-Length and Transfer-Encoding headers are not allowed twice */
                    if ((h->key == "host" || h->key == "content-length" || h->key == "transfer-encoding") && req->getHeader(h->key).data()) {
            ...
            if (transferEncodingString.length()) {
                ...
                if (!CONSUME_MINIMALLY) {
                    ...
                    for (auto chunk : uWS::ChunkIterator(&dataToConsume, &remainingStreamingBytes)) {
                        void *returnedUser = dataHandler(user, chunk, chunk.length() ? UINT64_MAX : 0);
                        if (returnedUser != user) {
                            return {consumedTotal, returnedUser};
                        }
                    }
                    consumeChunkedTrailer(dataToConsume);
                ...
                if (!CONSUME_MINIMALLY) {
                    ...
                    void *returnedUser = dataHandler(user, std::string_view(data, emittable), remainingStreamingBytes - emittable);
                    if (returnedUser != user) {
                        return {consumedTotal, returnedUser};
                    }
                ...
                } else if (remainingStreamingBytes == 0) {
                    /* Content-Length: 0 whose headers arrived fragmented: emit the terminal empty chunk now,
                     * otherwise onData/collectBody never fires and the request idle-times-out. */
    ...
    /* Consumes a streaming body chunk (chunked transfer-encoding or content-length) while
     * remainingStreamingBytes is set. Advances data/length and returns a pair to propagate
     * to the caller, or nullopt to continue parsing. */
    std::optional<std::pair<unsigned int, void *>> consumeStreamingBody(char *&data, unsigned int &length, void *user, MoveOnlyFunction<void *(void *, std::string_view, uint64_t)> &dataHandler) {
        ...
        if (isParsingChunkedEncoding(remainingStreamingBytes)) {
            ...
            for (auto chunk : uWS::ChunkIterator(&dataToConsume, &remainingStreamingBytes)) {
                ...
                void *returnedUser = dataHandler(user, chunk, chunk.length() ? UINT64_MAX : 0);
                if (returnedUser != user) {
                    return std::pair<unsigned int, void *>{0, returnedUser};
                }
            }
            consumeChunkedTrailer(dataToConsume);
            if (isParsingInvalidChunkedEncoding(remainingStreamingBytes)) {
                return std::pair<unsigned int, void *>{HTTP_ERROR_400_BAD_REQUEST, FULLPTR};
            }
            data = (char *) dataToConsume.data();
            length = (unsigned int) dataToConsume.length();
        } else {
...
public:
    std::pair<unsigned int, void *> consumePostPadded(char *data, unsigned int length, void *user, void *reserved, MoveOnlyFunction<void *(void *, HttpRequest *)> &&requestHandler, MoveOnlyFunction<void *(void *, std::string_view, uint64_t)> &&dataHandler) {
        ...
        if (remainingStreamingBytes) {
            ...
            if (auto propagate = consumeStreamingBody(data, length, user, dataHandler)) {
                return *propagate;
            }
