...
static inline size_t messageFrameSize(size_t messageSize) {
    /* Client frames carry a 4-byte masking key after the length */
    size_t maskSize = isServer ? 0 : 4;
    if (messageSize < 126) {
        return 2 + maskSize + messageSize;
    } else if (messageSize <= UINT16_MAX) {
        return 4 + maskSize + messageSize;
    }
    return 10 + maskSize + messageSize;
}
...
template <bool isServer>
static inline size_t formatMessage(char *dst, const char *src, size_t length, OpCode opCode, size_t reportedLength, bool compressed, bool fin, uint32_t mask = 0) {
    ...
    if (!isServer) {
        /* Mask the payload in place. 8-byte chunks then a byte tail so we never
         * write past dst + headerLength + length (the send buffer has no slack). */
        char *payload = dst + headerLength;
        uint64_t mask64;
        memcpy(&mask64, maskBytes, 4);
        memcpy((char *) &mask64 + 4, maskBytes, 4);
        size_t i = 0;
        for (; i + 8 <= length; i += 8) {
            uint64_t chunk;
        ...
        if (wState->state.wantsHead) {
            ...
            while (length >= SHORT_MESSAGE_HEADER) {
                ...
                if ((rsv1(src) && !Impl::setCompressed(wState, user)) || rsv23(src) || (getOpCode(src) > 2 && getOpCode(src) < 8) ||
                    ...
                    (!isServer && (((unsigned char *) src)[1] & 0x80))) {
                    Impl::forceClose(wState, user, ERR_PROTOCOL);
                    return;
                }
