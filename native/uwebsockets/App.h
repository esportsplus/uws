...
namespace uWS {
    ...
    struct SocketContextOptions {
        ...
        const char *ca_pem = nullptr;         /* in-memory PEM CA bundle; alternative to ca_file_name (client) */
        int client_mode = 0;                  /* 0 = server context, 1 = client context */
        int reject_unauthorized = 0;          /* client only; 1 = SSL_VERIFY_PEER, 0 = SSL_VERIFY_NONE */
...
public:
    ...
    std::vector<MoveOnlyFunction<void()>> listenSocketInvalidators;

    TopicTree<TopicTreeMessage, TopicTreeBigMessage> *topicTree = nullptr;

    /* Server name */
    bool addServerName(std::string hostname_pattern, SocketContextOptions options = {}) {
        ...
        if constexpr (SSL) {
            ...
            if (us_socket_context_add_server_name(SSL, (struct us_socket_context_t *) httpContext, hostname_pattern.c_str(), options, domainRouter)) {
                delete domainRouter;
                return false;
            }
        }
    ...
    TemplatedApp &&removeServerName(std::string hostname_pattern) {
        ...
        if (domainRouter) {
            us_socket_context_clear_server_name_userdata(SSL, (struct us_socket_context_t *) httpContext, hostname_pattern.c_str());
        }

        us_socket_context_remove_server_name(SSL, (struct us_socket_context_t *) httpContext, hostname_pattern.c_str());
        if (domainRouter) {
            Loop::get()->defer([domainRouter] {
    ...
    unsigned int numSubscribers(std::string_view topic) {
        ...
        if (t) {
            return (unsigned int) topicTree->numSubscribers(t);
        }
    ...
    ~TemplatedApp() {
        ...
        if (httpContext) {
            /* Contexts must outlive every socket using them. This normally happens in close(), but
             * environment teardown can destroy an App without an explicit close(). */
            if (!closed) {
                closed = true;
                us_socket_context_close(SSL, (struct us_socket_context_t *) httpContext);
                us_internal_socket_context_close_low_prio(SSL, (struct us_socket_context_t *) httpContext);
                for (void *webSocketContext : webSocketContexts) {
                    us_socket_context_close(SSL, (struct us_socket_context_t *) webSocketContext);
                    us_internal_socket_context_close_low_prio(SSL, (struct us_socket_context_t *) webSocketContext);
                }
            }
            httpContext->free();
    ...
    TemplatedApp(TemplatedApp &&other) {
        ...
        listenSocketInvalidators = std::move(other.listenSocketInvalidators);
    ...
    TemplatedApp(SocketContextOptions options = {}, size_t maxHeaderSize = DEFAULT_MAX_HEADER_SIZE) {
        ...
        this->any("/*", [](auto *res, auto */*req*/) {
            ...
            res->end("<html><body><h1>File Not Found</h1><hr><i>uws Server</i></body></html>");
        });
    }
    ...
    TemplatedApp &&close() {
        ...
        for (auto &listenSocketInvalidator : listenSocketInvalidators) {
            listenSocketInvalidator();
        }
        listenSocketInvalidators.clear();
