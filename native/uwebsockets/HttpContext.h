...
extern thread_local int insideRouteDispatch;
    ...
    HttpContext<SSL> *init() {
        ...
        us_socket_context_on_data(SSL, getSocketContext(), [](us_socket_t *s, char *data, int length) {
            ...
            auto [err, returnedSocket] = httpResponseData->consumePostPadded(data, (unsigned int) length, s, proxyParser, httpContextData->maxHeaderSize, [httpContextData](void *s, HttpRequest *httpRequest) -> void * {
                ...
                /* The request object is only valid while dispatching, so retain whether this is HEAD on the response. */
                if (httpRequest->getCaseSensitiveMethod() == "HEAD") {
                    httpResponseData->state |= HttpResponseData<SSL>::HTTP_METHOD_HEAD;
                }

                /* Mark this response as connectionClose on connection: close */
                std::string_view connection = httpRequest->getHeader("connection");
                if (connection.length() == 5 &&
                    (connection[0] == 'c' || connection[0] == 'C') &&
                    (connection[1] == 'l' || connection[1] == 'L') &&
                    (connection[2] == 'o' || connection[2] == 'O') &&
                    (connection[3] == 's' || connection[3] == 'S') &&
                    (connection[4] == 'e' || connection[4] == 'E')) {
                ...
                bool routed;
                {
                    ...
                    routed = selectedRouter->route(httpRequest->getCaseSensitiveMethod(), httpRequest->getUrl());
                }
                if (!routed) {
                    /* We have to force close this socket as we have no handler for it. URL segment overflows are handled by the any route. */
                    us_socket_close(SSL, (us_socket_t *) s, 0, nullptr);
                    return nullptr;
                }
                ...
                /* Returning from a request handler without responding or attaching an onAborted handler is
                 * ill-use: the connection can make no progress. Close this one socket and log, rather than
                 * aborting the whole process and taking every other connection down with it. */
                if (!((HttpResponse<SSL> *) s)->hasResponded() && !httpResponseData->onAborted) {
                    std::cerr << "Error: Returning from a request handler without responding or attaching an abort handler is forbidden; closing this connection."
                              << std::endl
            ...
            if (httpContextData->upgradedWebSocket) {
                ...
                if (!failed) {
                    WebSocketData<true> *webSocketData = (WebSocketData<true> *) asyncSocket->getAsyncSocketData();
...
public:
    ...
    void onHttp(std::string method, std::string pattern, MoveOnlyFunction<void(HttpResponse<SSL> *, HttpRequest *)> &&handler, bool upgrade = false) {
        ...
        httpContextData->currentRouter->add(method, pattern, [handler = std::move(handler), parameterOffsets = std::move(parameterOffsets)](auto *r) mutable {
