...
private:
    ...
    static bool setCompressed(WebSocketState<isServer> */*wState*/, void *s) {
        WebSocketData<isServer> *webSocketData = (WebSocketData<isServer> *) us_socket_ext(SSL, (us_socket_t *) s);

        if (webSocketData->compressionStatus == WebSocketData<isServer>::CompressionStatus::ENABLED) {
            webSocketData->compressionStatus = WebSocketData<isServer>::CompressionStatus::COMPRESSED_FRAME;
            return true;
        } else {
            return false;
        }
    ...
    static bool handleFragment(char *data, size_t length, unsigned int remainingBytes, int opCode, bool fin, WebSocketState<isServer> *webSocketState, void *s) {
        ...
        if (opCode < 3) {
            ...
            if (!remainingBytes && fin && !webSocketData->fragmentBuffer.length()) {
                ...
                if (webSocketData->compressionStatus == WebSocketData<isServer>::CompressionStatus::COMPRESSED_FRAME) {
                        webSocketData->compressionStatus = WebSocketData<isServer>::CompressionStatus::ENABLED;
    ...
    WebSocketContext<SSL, isServer, USERDATA> *init() {
        ...
        us_socket_context_on_close(SSL, getSocketContext(), [](auto *s, int code, void *reason) {
#ifdef _WIN32
            Loop::get()->removePostHandler(s);
#endif
            /* For whatever reason, if we already have emitted close event, do not emit it again */
            WebSocketData<isServer> *webSocketData = (WebSocketData<isServer> *) (us_socket_ext(SSL, s));
            if (!webSocketData->isShuttingDown) {
                webSocketData->isShuttingDown = true; /* Set before emitting so a re-entrant end()/close() from a subscription or close handler is a no-op instead of a second emit. */
                /* Emit close event */
                auto *webSocketContextData = (WebSocketContextData<SSL, isServer, USERDATA> *) us_socket_context_ext(SSL, us_socket_context(SSL, (us_socket_t *) s));
                ...
                if (webSocketData->subscriber && webSocketContextData->subscriptionHandler) {
                    ...
                    for (std::string &name : names) {
                        ...
                        webSocketContextData->subscriptionHandler((WebSocket<SSL, isServer, USERDATA> *) s, name, count - 1, count);
                    }
                }
            ...
            /* Transport close is complete and the socket is about to be freed: notify anyone tracking the
             * socket's true lifetime (the outbound client retires its per-connect slot here). Fetched
             * regardless of isShuttingDown so the graceful end() path - which emits closeHandler early while
             * the socket is still open - still retires only now. Null on server contexts. */
            auto *closingContextData = (WebSocketContextData<SSL, isServer, USERDATA> *) us_socket_context_ext(SSL, us_socket_context(SSL, (us_socket_t *) s));
            if (closingContextData->socketClosedHandler) {
                closingContextData->socketClosedHandler();
            }
        ...
        us_socket_context_on_data(SSL, getSocketContext(), [](auto *s, char *data, int length) {
            /* The entire body lives in the public consume() so the client handshake context can feed
             * bytes that arrived in the same read as the 101 response (spec A.3b). */
            WebSocketContext<SSL, isServer, USERDATA>::consume(s, data, length);
            return s;
        });
        ...
        us_socket_context_on_timeout(SSL, getSocketContext(), [](auto *s) {
            ...
            auto *webSocketData = (WebSocketData<isServer> *)(us_socket_ext(SSL, s));
            auto *webSocketContextData = (WebSocketContextData<SSL, isServer, USERDATA> *) us_socket_context_ext(SSL, us_socket_context(SSL, (us_socket_t *) s));
            ...
            if (webSocketContextData->sendPingsAutomatically && !webSocketData->isShuttingDown && !webSocketData->hasTimedOut) {
                ...
                /* Send ping without being corked. A client MUST mask every frame (RFC 6455 5.3), so the
                 * outbound role sends an empty masked ping (FIN|PING, MASK|len0, 4-byte key; empty payload
                 * needs no XOR) rather than the server's bare unmasked frame - an unmasked client ping is a
                 * protocol error a conformant server drops. */
                if constexpr (isServer) {
                    ((AsyncSocket<SSL> *) s)->write("\x89\x00", 2);
                } else {
                    ((AsyncSocket<SSL> *) s)->write("\x89\x80\x00\x00\x00\x00", 6);
                }
                return s;
            }
...
public:
    /* Runs the WebSocket on_data body for a socket already living on this context. Public so the client
     * handshake context (WebSocketClientContext) can push the bytes that trailed the 101 response in the
     * same read into the freshly adopted frame-phase socket (spec A.3b, HttpContext.h:131 "clients need
     * to know the cursor after http parse"). Identical to the registered on_data lambda body. */
    static void consume(us_socket_t *s, char *data, int length) {
