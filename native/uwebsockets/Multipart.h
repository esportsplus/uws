...
namespace uWS {
    ...
    struct MultipartParser {
        ...
        MultipartParser(std::string_view contentType) {
            ...
            auto boundaryToken = contentType.find("boundary=", 10);
            if (boundaryToken != std::string_view::npos) {

                /* Boundary must be less than or equal to 70 chars yet 1 char or longer */
                std::string_view boundary = contentType.substr(boundaryToken + 9);
                boundary = boundary.substr(0, boundary.find(';'));

                auto first = boundary.find_first_not_of(" \t");
                if (first == std::string_view::npos) {
                    return;
                }
                boundary = boundary.substr(first, boundary.find_last_not_of(" \t") - first + 1);
