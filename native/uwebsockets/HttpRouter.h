...
private:
    ...
    inline void setUrl(std::string_view url) {
        ...
        currentUrl = url;
        urlSegmentTop = -1;
        urlTooLong = std::count(url.begin(), url.end(), '/') > MAX_URL_SEGMENTS;
    }
...
public:
    ...
    bool route(std::string_view method, std::string_view url) {
        ...
        /* Segment overflows still get the normal any-route fallback. */
        if (urlTooLong) {
            urlTooLong = false;
    ...
    void add(std::string method, std::string pattern, MoveOnlyFunction<bool(HttpRouter *)> &&handler, uint32_t priority = MEDIUM_PRIORITY) {
        /* First remove existing handler */
        remove(method, pattern, priority);
